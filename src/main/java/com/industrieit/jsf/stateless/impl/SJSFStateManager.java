...
public class SJSFStateManager extends com.sun.faces.application.StateManagerImpl {
	...
	@Override public String getViewState(FacesContext context) {
	...
	@Override public boolean isSavingStateInClient(FacesContext context) {
		if (SJSFUtil.isPoolable(context.getViewRoot())) {
			return false;
		} else {
	...
	@Override public Object saveView(FacesContext context) {
		if (SJSFUtil.isPoolable(context.getViewRoot())) {
			return "SJSF";
		} else {
	...
	@Override public void writeState(FacesContext context, SerializedView state) throws IOException {
		if (SJSFUtil.isPoolable(context.getViewRoot())) {
			//nothing
		} else {
