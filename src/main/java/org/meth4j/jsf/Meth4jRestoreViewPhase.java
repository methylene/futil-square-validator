 ...
 * https://svn.java.net/svn/mojarra~svn/trunk/jsf-ri/src/main/java/com/sun/faces/lifecycle/RestoreViewPhase.java
 * 
 * 
...
public class Meth4jRestoreViewPhase extends Phase {
...
//                facesContext.renderResponse(); // TODO methylene: commented out a line to fix f:viewParam when using Stateless JSF
//            }
            return;
        }
        ...
        try {
            ...
            final ViewHandler viewHandler = Util.getViewHandler(facesContext);

            final boolean isPostBack = (facesContext.isPostback() && !isErrorPage(facesContext));
            if (isPostBack) {
            ...
            // try to restore the view
                ...
                final String derivedViewId = viewHandler.deriveLogicalViewId(facesContext, viewId);
                final ViewDeclarationLanguage vdl = viewHandler.getViewDeclarationLanguage(facesContext, derivedViewId);
                
                maybeTakeProtectedViewAction(facesContext, viewHandler, vdl, viewId);
                                
                if (vdl != null) {
                    // If we have one, get the ViewMetadata...
                    final ViewMetadata metadata = vdl.getViewMetadata(facesContext, viewId);
                    ...
                    if (metadata != null) { // perhaps it's not supported
                        ...
                        final Collection<UIViewParameter> params =
                                ViewMetadata.getViewParameters(viewRoot);
                        if (params.isEmpty()) {
                            final UIComponent metaDataFacet = viewRoot.getFacets().get(UIViewRoot.METADATA_FACET_NAME);
    ...
    private void maybeTakeProtectedViewAction(FacesContext context, 
        ...
        final Set<String> urlPatterns = viewHandler.getProtectedViewsUnmodifiable();
        // Implement section 12.1 of the Servlet spec.
        boolean currentViewIsProtected = false;
        for (final String cur : urlPatterns) {
        ...
        if (currentViewIsProtected) {
            ...
            final Map<String, String> headers = extContext.getRequestHeaderMap();
...
//                    }
                    ...
                    if (!refererOriginatesInThisWebapp) {
                        final String message = FacesLogger.LIFECYCLE.interpolateMessage(context, 
                ...
                final String rkId = viewHandler.calculateRenderKitId(context);
                final ResponseStateManager rsm = RenderKitUtils.getResponseStateManager(context, rkId);
                final String incomingSecretKeyValue = extContext.getRequestParameterMap().
    ...
    private void deliverPostRestoreStateEvent(FacesContext facesContext) throws FacesException {
        ...
        try {
            ...
            final Set<VisitHint> hints = EnumSet.of(VisitHint.SKIP_ITERATION);
            final VisitContext visitContext = VisitContext.createVisitContext(facesContext, null, hints);
    ...
    private void notifyAfter(FacesContext context, Lifecycle lifecycle) {
        ...
        final MethodExpression afterPhase = viewRoot.getAfterPhaseListener();
        if (null != afterPhase) {
            try {
                final PhaseEvent event = new PhaseEvent(context, PhaseId.RESTORE_VIEW, lifecycle);
